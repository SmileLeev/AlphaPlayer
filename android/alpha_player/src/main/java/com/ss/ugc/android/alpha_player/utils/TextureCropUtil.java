...
public class TextureCropUtil {
    ...
    public static float[][] calculateVerticeData(float[] verticeData,float[] textureCoord,ScaleType scaleType,
                                                        float viewWidth, float viewHeight,
                                                        float videoWidth, float videoHeight) {
        float[][] result = new float[3][];
        result[0] = Arrays.copyOf(verticeData, verticeData.length);
    ...
    private static float[] getZoomData(float[] verticeData, float leftZoomRatio, float topZoomRatio, float rightZoomRatio, float bottomZoomRatio) {
        ...
        verticeData[lineCount+1] += bottomZoomRatio * 2;
        verticeData[lineCount*2] += leftZoomRatio * 2;
        verticeData[lineCount*2+1] -= topZoomRatio * 2;
        verticeData[lineCount*3] -= rightZoomRatio * 2;
        verticeData[lineCount*3+1] -= topZoomRatio * 2;
        return verticeData;
//        return new float[] {
//                // X, Y, Z, U, V
